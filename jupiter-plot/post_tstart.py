...
def visit_writes(set_paths, function, t_start, **kw):
    """
    ...
    arg_list = zip(set_paths, *get_assigned_writes(set_paths, t_start))
...
def get_assigned_writes(set_paths, t_start):
    """
    ...
    writes, offset = get_all_writes(set_paths, t_start)
    block = int(np.ceil(sum(writes) / MPI_SIZE))
    proc_start = offset + MPI_RANK * block
    # Find set start/end indices
    writes = np.array(writes)
    set_ends = offset + np.cumsum(writes)
    ...
    return starts-set_starts+offset, counts


def get_all_writes(set_paths, t_start):
    """
    ...
    for set_path in set_paths:
        with h5py.File(str(set_path), mode='r') as file:
            ...
            writes_discard = np.sum(times <= t_start)
            writes.append(file.attrs['writes'] - writes_discard)
    return writes, writes_discard
