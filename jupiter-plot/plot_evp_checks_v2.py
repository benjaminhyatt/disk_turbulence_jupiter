...
#psi_evecs_res_sorted = processed['psi_right_evecs_res'][drift_sort, :, :]
vort_evecs_res_sorted = processed['vort_right_evecs_res'][drift_sort, :, :]
...
for i in range(n_modes):
    ...
    #psi['g'] = np.copy(psi_evecs_res_sorted[i, :, :])
    #ax.pcolormesh(x, y, psi['g'].real, cmap=cmap)
    #ax.set_title(r"$\psi$, $\omega =$" + "{:.3}".format(evals_res_sorted[i].real) + r"$+ i$" + "{:.3}".format(evals_res_sorted[i].imag))
