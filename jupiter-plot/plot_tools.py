...
def plot_bot(dset, image_axes, data_slices, image_scales=(0,0), clim=None, even_scale=False, cmap='RdBu_r', axes=None, figkw={}, title=None, func=None, visible_axes=True, normopt=None, thresh=None, cmax=None):
    """
    ...
    normopt : str corresponding to a type of matplotlib.colors object, optional; currently only 'symlog' is implemented (default: None)
        norm to rescale colors in pcolormesh (and colormap)
    thresh : float, optional (default: None)
        linthresh for 'symlog' normopt
    cmax : float, optional (default: None)
        max value for 'symlog' normopt
    """
    ...
    if isinstance(axes, (list, tuple)):
        ...
        pbbox = transforms.Bbox.from_bounds(0.03, 0.06, 0.94, 0.93)
        cbbox = transforms.Bbox.from_bounds(0.03, 0, 0.94, 0.05)
    ...
    if normopt is not None:
        if normopt == 'symlog':
            scl = 1
            if cmax is None:
                cmax = max(abs(data.min()), abs(data.max()))
            normsl = matplotlib.colors.SymLogNorm(linthresh=thresh, linscale=scl, vmin=-cmax, vmax=cmax, base=10)
            plot = paxes.pcolormesh(xmesh, ymesh, data, cmap=cmap, zorder=1, norm=normsl)
    else:
    ...
    if normopt is not None:
        if normopt == 'symlog':
            cbar = plt.colorbar(plot, cax=caxes, orientation='horizontal', ticks=ticker.SymmetricalLogLocator(base=10, linthresh=thresh))
            caxes.xaxis.minorticks_off() 
    else:
        cbar = plt.colorbar(plot, cax=caxes, orientation='horizontal',
            ticks=ticker.MaxNLocator(nbins=5))
    cbar.outline.set_visible(False)
    caxes.xaxis.set_ticks_position('bottom')
    ...
    caxes.xaxis.set_label_position('bottom')
