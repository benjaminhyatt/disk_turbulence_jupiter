...
Produces:
  1. A time-averaged profile of u_phi^CPC and u_r^CPC vs r (distance from pole,
     along the radial slice through the CPC center).
  2. Optionally, a vertical dashed line at r_CPC (mean tracked radius).
...
### figure style (matching plot_profiles_u.py) ###
dpi = 300
...
ax1.axvline(r_CPC, color='k', ls='--', lw=1.0, label=f'$r_{{\\rm CPC}}={r_CPC:.4f}$')
ax1.plot(r_profile_pts, uphi_CPC_mean, color='C0', lw=2.0,
         label=r'$\langle u_\phi^{\rm CPC} \rangle_t$')
ax1.set_xlabel(r'$r$')
ax1.set_ylabel(r'$u_\phi^{\rm CPC}$')
ax1.legend(loc='lower right', fontsize=9)
ax1.set_title(f'CPC velocity profile  ($N_{{\\rm frames}}={n_vel_profile}$)', fontsize=10)

### panel 2: u_r^CPC ###
ax2 = fig.add_axes([left, bottom2, width, height])
ax2.axhline(0., color='gray', lw=0.8)
ax2.axvline(r_CPC, color='k', ls='--', lw=1.0, label=f'$r_{{\\rm CPC}}={r_CPC:.4f}$')
ax2.plot(r_profile_pts, ur_CPC_mean, color='C1', lw=2.0,
         label=r'$\langle u_r^{\rm CPC} \rangle_t$')
ax2.set_xlabel(r'$r$')
