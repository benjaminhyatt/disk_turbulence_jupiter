...
### figure ###
fig, axes = plt.subplots(1, 1, figsize=(7.5, 5), constrained_layout=True)
...
# various predictions...
urms_list =    np.array([ 3.59,  3.61,  3.52,  3.38,  3.11,  2.76,  2.66,  2.48,  2.57,  2.56])
eps_eff_list = np.array([0.405, 0.410, 0.390, 0.359, 0.304, 0.240, 0.223, 0.194, 0.208, 0.206])
lgams = (urms_list / gamma_vals)**(1/3)
ltrs = 1 / gamma_vals**(3/8)
ltrs_rs = ltrs * (2**(3/8) * (np.pi)**(1/4))
ltrs_eff = eps_eff_list**(1/8) / gamma_vals**(3/8)
ltrs_eff_rs = ltrs_eff * (2**(3/8) * (np.pi)**(1/4))

om_lgams = 1 * gamma_vals * lgams**2
om_ltrs = 1 * gamma_vals * ltrs**2
om_ltrs_rs = 1 * gamma_vals * ltrs_rs**2
om_ltrs_eff = 1 * gamma_vals * ltrs_eff**2
om_ltrs_eff_rs = 1 * gamma_vals * ltrs_eff_rs**2


ax.plot(gamma_vals, om_lgams, linestyle='dotted', label = r'based on $L_{\gamma}$')
ax.plot(gamma_vals, om_ltrs, linestyle='dotted', label = r'based on $L_{tr}$')
ax.plot(gamma_vals, om_ltrs_rs, linestyle='dotted', label = r'based on $L_{tr}$, rescaled')
ax.plot(gamma_vals, om_ltrs_eff, linestyle='dotted', label = r'based on $L_{tr}$, $\epsilon_{eff}$')
ax.plot(gamma_vals, om_ltrs_eff_rs, linestyle='dotted', label = r'based on $L_{tr}$, $\epsilon_{eff}$, rescaled')
