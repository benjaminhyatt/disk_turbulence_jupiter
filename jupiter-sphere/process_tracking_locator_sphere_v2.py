...
Options:
    ...
    --n_rho_fit=<int>        number of rho points (rho=0 prepended) [default: 192]
    --n_alpha_fit=<int>      number of alpha points (uniform) [default: 64]
    --R_threshold_half=<float>   [default: 0.5]
    --R_threshold_fifth=<float>  [default: 0.2]
    --R_threshold_tenth=<float>  [default: 0.1]

    --bin_width_phi=<float>      phi bin width (rad) [default: 0.02]
...
### --- set outer colatitude (where to cut off the polar search) --- ###
...
if cap_colat_arg.lower() == 'auto':
    ...
    if gamma <= 0:
        ...
        # CPCs can roam freely.  Edge case: if both CPCs land in the same
        # hemisphere, the script will report whichever is currently larger.
        # If the edge case turns out to be a frequent occurrence, we can revisit.
        L_rh      = np.nan
        cap_colat = np.pi / 2
        print(f"cap_colat (auto, gamma=0): hemispheric, "
              f"cap_colat=pi/2 ({cap_colat:.6f})")
    else:
        C = U_rms / gamma
        ...
        # too tight when the rotational confinement is very strong),
        # and ceilinged by pi/2 (hemispheric upper bound).
        cap_colat = float(min(max(2 * L_rh, min_cap_colat), np.pi / 2))
        print(f"cap_colat (auto): U_rms={U_rms:.6f}, "
              f"L_rh_flat={L_rh_flat:.6f}, L_rh_sphere={L_rh:.6f}, "
              f"min_cap_colat={min_cap_colat:.6f}, "
              f"cap_colat={cap_colat:.6f}")
else:
    cap_colat = float(cap_colat_arg)
    if cap_colat > np.pi / 2:
        cap_colat = np.pi / 2
        print(f"  warning: cap_colat capped at pi/2 (hemispheric upper bound)")
    print(f"cap_colat (CLI): {cap_colat:.6f}")
...
full_basis = d3.SphereBasis(coords, (Nphi_grid, Ntheta_grid),
                             radius=1.0, dealias=dealias, dtype=np.float64)
phi_grid_d3, theta_grid_d3 = dist.local_grids(full_basis, scales=(1, 1))
# convention: phi runs 0 (inclusive) -> 2*pi (exclusive), theta descends from
...
for i, w in enumerate(ws):
    ...
    # current rms enstrophy (defines one of the CPC length scales)
    vort_rms_t = np.sqrt(d3.Average(vort_field * vort_field).evaluate()['g'].flat[0])
    ...
    for p in range(n_poles):
        # grid-point extremum (coarse)
        phi_idx, theta_idx, _, sign = find_max_abs_in_cap(vort_g, cap_idxs_per_pole[p])

        # sub-grid refinement via local spline (RectSphereBivariateSpline +  
        ...
        # CPC-frame alpha-averaged (azimuthally-averaged in CPC frame) radial profile
        ...
        # 2D position histogram (Note: digitize returns indices for a given prescription of bin edges)
