...
Options:
    ...
    --gamma=<str>            gamma (= 2*Omega/R^2 = 2*Omega for R=1); 'auto' uses 2*1186 [default: auto]
    --alpha=<float>          friction [default: 0.0333333]
    --eps=<float>            energy injection rate [default: 1.0]

    --cap_colat=<str>        outer colatitude of each polar cap (rad), clamped to <= pi/2; 
                             'auto' uses max(2 * (spherical Rhines angle), min_cap_colat) for gamma>0, 
                             or pi/2 (hemispheric) for gamma==0 [default: auto]
    --min_cap_colat=<float>  minimum cap_colat (rad) used when 2*L_rh would be smaller; rotating cases only.  
                             Default 0.2618 ~ pi/12 ~ 15 deg. Overridden by the hemispheric setting when 
                             gamma == 0 [default: 0.2618]
    ...
    --R_threshold_1=<float>   [default: 0.5]
    --R_threshold_2=<float>  [default: 0.2]
    --R_threshold_3=<float>  [default: 0.1]

    --bin_width_phi=<float>      phi histogram bin width (rad) [default: 0.05]
    --bin_width_theta=<float>    theta histogram bin width (rad) [default: 0.005]
"""
...
from tracking_locator_sphere_helpers import (
...
R_thresh_0  = float(args['--R_threshold_0'])
R_thresh_1 = float(args['--R_threshold_1'])
R_thresh_2 = float(args['--R_threshold_2'])
...
peak_thresholds = np.array([R_thresh_0, R_thresh_1, R_thresh_2])
