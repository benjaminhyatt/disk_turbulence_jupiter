...
The most useful keys, organized by topic:
  ...
  CPC length scales  (R_type index: 0 = R_half, 1 = R_fifth, 2 = R_tenth, 3 = R_rms):
  ...
  Antipodality diagnostics (each is shape (frame,)):
    antipodal_gc_dist      - great-circle distance between CPC_N and *the antipode* of CPC_S (zero if perfectly antipodal)
    antipodal_delta_theta  - theta_N + theta_S - pi (the diff in each CPC's colat. separation from its own pole)
    antipodal_delta_phi    - wrap(phi_N - phi_S - pi) (zero if antipodal)
    relative_zonal_phase   - wrap(phi_N - phi_S) (pi if antipodal)
    antipodal_stats[etric, stat] - mean/std deviations for each of the above, indexed 0 to 3

  Convention notes:
    Colatitude theta in [0, pi] with theta=0 at "north"
    Pole index 0 = small theta (N), pole index 1 = large theta (S).

--- Usage instructions ---
Usage:
    process_tracking_locator_sphere.py <hdf5_file> [options]

The <hdf5_file> argument accepts either a single HDF5 file or a glob pattern (e.g. 'snapshots/snapshots_s*.h5');
matched files are sorted by their trailing integer index and their writes are processed consecutively.
...
Options:
    ...
    --plot=<str>             POSITION figure path; 'auto'; 'none' to skip [default: auto]
    --plot_amp=<str>         AMPLITUDE/PROFILE figure path; 'auto'; 'none' to skip [default: auto]
...
def extract_output_suffix(file_path):
    ...
    for prefix in ('snapshots_', 'snapshots'):
        if basename.startswith(prefix):
            basename = basename[len(prefix):].lstrip('_')
            break
...
plot_path = None if plot_arg.lower() in ('none', '') else path_or_default(plot_arg, f"{output_prefix}_position_{output_suffix}.png")
plot_amp_path = None if plot_amp_arg.lower() in ('none', '') else path_or_default(plot_amp_arg, f"{output_prefix}_amplitude_{output_suffix}.png")
...
if cap_colat_arg.lower() == 'auto':
    ...
    if gamma <= 0:
        # Use hemispherical caps so the CPCs can roam freely.  
        # Note: if both CPCs land in the same hemisphere, 
        # the script will still record a "second" extremum in the empty hemisphere 
        # -- can spot this by comparing |vort_peak| between the two recorded poles.
        L_rh = np.nan
        cap_colat = np.pi / 2
        print(f"cap_colat (auto, gamma=0): hemispheric, cap_colat=pi/2 ({cap_colat:.4f})")
    else:
        C = U_rms / gamma
        ...
        if rhines_eq(np.pi / 2) < 0:
            ...
            # quick nonlinear solve
            L_rh = float(brentq(rhines_eq, 1e-6, np.pi / 2))
        # cap is 2*L_rh, floored by min_cap_colat and ceilinged by pi/2
        cap_colat = float(min(max(2 * L_rh, min_cap_colat), np.pi / 2))
        print(f"cap_colat (auto): U_rms={U_rms:.4f}, L_rh_flat={L_rh_flat:.4f}, L_rh_sphere={L_rh:.4f}, min_cap_colat={min_cap_colat:.4f}, cap_colat={cap_colat:.4f}")
else:
    cap_colat = float(cap_colat_arg)
    if cap_colat > np.pi / 2:
        cap_colat = np.pi / 2
        print(f"warning: cap_colat capped at pi/2 (hemispheric upper bound)")
    print(f"cap_colat (specified as input): {cap_colat:.4f}")
...
print(f"HDF5: {len(files)} file(s), {len(t_all)} total writes, t in [{t_all[0]:.3f}, {t_all[-1]:.3f}], grid ({Nphi_grid} phi, {Ntheta_grid} theta)")
...
full_basis = d3.SphereBasis(coords, (Nphi_grid, Ntheta_grid), radius=1.0, dealias=3/2, dtype=np.float64)
...
print(f"cap sizes:  N={len(cap_idxs_per_pole[0])}, S={len(cap_idxs_per_pole[1])}")
...
print(f"tavg_window: '{tavg_window_src}' -> t in [{tavg_t0:.3f}, {tavg_t1:.3f}] ({n_tavg}/{nw} frames)")
if n_tavg == 0:
...
rho_fit_grid = np.concatenate([[0.0], np.logspace(np.log10(rho_min_fit), np.log10(rho_max_fit), n_rho_fit - 1)])
...
for i, w in enumerate(ws):
    ...
    vort_rms_t = float(np.sqrt(
        max(float(d3.Average(vort_field * vort_field).evaluate()['g'].flat[0]), 0.0)))
    ...
    interp_vort = RegularGridInterpolator((phi_1d_wrap, theta_1d), vort_wrap, method='linear', bounds_error=False, fill_value=np.nan)

    for p in range(n_poles):
        # extremum over grid points
        phi_idx, theta_idx, _, sign = find_max_abs_in_cap(vort_g, cap_idxs_per_pole[p])
        # use spline for better estimate
        theta_p, phi_p, vort_val = refine_extremum_via_spline(theta_1d, phi_1d, vort_g, phi_idx, theta_idx)
        ...
        theta_lab, phi_lab = cpc_frame_to_lab(theta_p, phi_p, rho_fit_grid, alpha_fit_grid)
        ...
        if np.isfinite(peak_val) and abs(peak_val) > 0:
            for k, frac in enumerate(peak_thresholds):
                R_history[p, k, i] = find_threshold_crossing(rho_fit_grid, vort_radial, frac * peak_val)
            R_history[p, 3, i] = find_threshold_crossing(rho_fit_grid, vort_radial, sign * vort_rms_t)
            # only accumulate the time-avg radial profile over the selected tavg_window
        ...
        if tavg_mask[i]:
            phi_bin = max(0, min(int(np.digitize(phi_p % (2 * np.pi), phi_bin_edges)) - 1, n_phi_bins - 1))
            theta_bin = max(0, min(int(np.digitize(theta_p, theta_bin_edges)) - 1, n_theta_bins - 1))
...
antipodal_gc_dist = great_circle_distance(theta_N, phi_N, theta_S_antipode, phi_S_antipode)
antipodal_delta_theta = theta_N + theta_S - np.pi
antipodal_delta_phi = np.arctan2(np.sin(phi_N - phi_S - np.pi), np.cos(phi_N - phi_S - np.pi))
relative_zonal_phase  = np.arctan2(np.sin(phi_N - phi_S), np.cos(phi_N - phi_S))
...
antipodal_stat_labels = ['gc_dist', 'delta_theta', 'delta_phi', 'relative_zonal_phase']
...
# Folded S colatitude (distance-from-own-pole) used in the overlapping trajectory and histogram plots.
...
def _shade_tavg(ax):
    ...
    ax.axvspan(tavg_t0, tavg_t1, color='gray', alpha=0.08, lw=0, zorder=0)

def _polar_trajectory(ax, phi, r, t, cmap='viridis', lw=0.7, alpha=0.65):
    """Time-colored polyline on a polar axis connecting consecutive (phi, r).
    Segments whose |Δphi| > π are treated as crossings of the phi=0/2π
    boundary and split at that boundary into two pieces (with r and t linearly
    interpolated to the crossing), so the path stays continuous through the
    coordinate seam rather than drawing a chord across the plot or being
    elided. NaN points in r or phi end the current sub-path."""
    phi = np.asarray(phi, dtype=float); r = np.asarray(r, dtype=float); t = np.asarray(t, dtype=float)
    two_pi = 2 * np.pi
    segs, cols = [], []
    for i in range(len(phi) - 1):
        ...
        r1, r2 = r[i], r[i+1]
        t1, t2 = t[i], t[i+1]
        if not (np.isfinite(p1) and np.isfinite(p2) and np.isfinite(r1) and np.isfinite(r2)):
            continue
        dphi = p2 - p1
        if abs(dphi) < np.pi:
            segs.append([[p1, r1], [p2, r2]]); cols.append(t1)
            continue
        # wraparound — pick the short direction and split at the boundary
        if dphi > 0:
            # short way is decreasing phi: p1 → 0, then 2π → p2
            d1, d2 = p1, two_pi - p2          # arclengths of the two sub-segments
            f = d1 / (d1 + d2)
            r_b = r1 + f * (r2 - r1); t_b = t1 + f * (t2 - t1)
            segs.append([[p1, r1], [0.0,    r_b]]); cols.append(t1)
            segs.append([[two_pi, r_b], [p2, r2]]); cols.append(t_b)
        else:
            # short way is increasing phi: p1 → 2π, then 0 → p2
            d1, d2 = two_pi - p1, p2
            f = d1 / (d1 + d2)
            r_b = r1 + f * (r2 - r1); t_b = t1 + f * (t2 - t1)
            segs.append([[p1, r1], [two_pi, r_b]]); cols.append(t1)
            segs.append([[0.0, r_b], [p2,    r2]]); cols.append(t_b)
    if len(segs) == 0:
        return None
    lc = LineCollection(np.asarray(segs), cmap=cmap, lw=lw, alpha=alpha)
    lc.set_array(np.asarray(cols))
    ax.add_collection(lc)
    return lc

### =========================== POSITION FIGURE =========================== ###
# Layout: row 0 has the consolidated phi histogram (single col) + the two polar
# trajectory plots; row 1 puts the colatitude histogram + the wide folded-theta
# time series (spanning 2 cols, so it has horizontal room to breathe); row 2 is
# the three antipodality time-series panels.
if plot_path is not None:
    print(f"Saving position plot: {plot_path}")
    fig = plt.figure(figsize=(15, 12), constrained_layout=True)
    gs = fig.add_gridspec(3, 3)

    # (0, 0) — consolidated 1D phi histogram for both poles, overlaid
    ax = fig.add_subplot(gs[0, 0])
    phi_bins_plot = np.linspace(0, 2 * np.pi, 40)
    for p in range(n_poles):
        sub = phi_locs[p][tavg_mask]
        sub = sub[np.isfinite(sub)]
        ax.hist(sub, bins=phi_bins_plot, alpha=0.55, color=pole_colors[p], label=pole_names[p], density=True)
    ax.set_xlim([0, 2 * np.pi])
    ax.set_xticks(np.arange(0, 2 * np.pi + 0.01, np.pi / 2))
    ax.set_xticklabels(['0', r'$\pi/2$', r'$\pi$', r'$3\pi/2$', r'$2\pi$'])
    ax.set_xlabel(r'$\varphi$ (rad)'); ax.set_ylabel('density')
    ax.set_title(r'1D $\varphi$ histogram (tavg window)')
    ax.legend(fontsize=8); ax.grid(True, alpha=0.3)

    # (0, 1) / (0, 2) — polar time-trajectory per pole (LineCollection
    # underneath the scatter so it reads as a directed path; consecutive
    # points are connected by a segment colored by start-time).
    for p, gs_col in [(0, 1), (1, 2)]:
        ax = fig.add_subplot(gs[0, gs_col], projection='polar')
        radial = theta_folded[p]
        _polar_trajectory(ax, phi_locs[p], radial, tw, cmap='viridis', lw=0.7, alpha=0.6)
        sc = ax.scatter(phi_locs[p], radial, s=2, c=tw, cmap='viridis', alpha=0.65)
        ax.set_ylim([0, theta_fold_ylim[1]])
        ax.set_title(fr'{pole_names[p]}: $(\varphi, $' + (r'$\theta_N$' if p == 0 else r'$\pi-\theta_S$') + r'$)$  trajectory', fontsize=10)
        cb = fig.colorbar(sc, ax=ax, pad=0.08, fraction=0.04)
        cb.set_label('sim time t', fontsize=8)

    # (1, 0) — 1D theta histogram (folded), both poles overlaid
    ax = fig.add_subplot(gs[1, 0])
    th_bins = np.linspace(0, theta_fold_ylim[1], 50)
    for p in range(n_poles):
        sub = theta_folded[p][tavg_mask]
        sub = sub[np.isfinite(sub)]
        ax.hist(sub, bins=th_bins, alpha=0.55, color=pole_colors[p], label=fr'{pole_names[p]}: {fold_labels[p]}', density=True)
    ax.set_xlabel('colat from own pole (rad)'); ax.set_ylabel('density')
    ax.set_title(r'1D colatitude histogram (tavg window)')
    ax.legend(fontsize=8); ax.grid(True, alpha=0.3)

    # (1, 1:3) — folded θ trajectories, spanning two columns for breathing room
    ax = fig.add_subplot(gs[1, 1:3])
    for p in range(n_poles):
        ax.plot(tw, theta_folded[p], color=pole_colors[p], lw=0.6, label=fold_labels[p])
    ax.axhline(cap_colat, color='gray', ls=':', lw=0.7, label=fr'cap_colat = {cap_colat:.3f}')
    _shade_tavg(ax)
    ...
    ax.axhline(antipodal_stats[0, 0], color='C2', ls='--', lw=1.0, label=fr'{antipodal_stats[0, 0]:.3f}')
    _shade_tavg(ax)
    ax.set_xlabel('t'); ax.set_ylabel('great-circle dist (rad)')
    ax.set_title(r'Antipodal distance')
    ax.legend(fontsize=8); ax.grid(True, alpha=0.3)
    ...
    ax.axhline(antipodal_stats[1, 0], color='C0', ls='--', lw=1.0, label=fr'{antipodal_stats[1, 0]:+.3f}')
    _shade_tavg(ax)
    ax.set_xlabel('t'); ax.set_ylabel(r'$\theta_N + \theta_S - \pi$ (rad)')
    ax.set_title(r'Colatitude mismatch')
    ax.legend(fontsize=8); ax.grid(True, alpha=0.3)

    # (2, 2) — wrap(phi_N - phi_S)
    ax = fig.add_subplot(gs[2, 2])
    ax.plot(tw, relative_zonal_phase, color='C4', lw=0.6)
    ax.axhline(0, color='gray', lw=0.5)
    ax.axhline(antipodal_stats[3, 0], color='C4', ls='--', lw=1.0, label=fr'{antipodal_stats[3, 0]:+.3f}')
    _shade_tavg(ax)
    ax.set_xlabel('t'); ax.set_ylabel(r'$\mathrm{wrap}(\varphi_N - \varphi_S)$ (rad)')
    ax.set_title(r'Longitudinal separation:  $\mathrm{wrap}(\varphi_N - \varphi_S)$')
    ax.set_ylim([-np.pi, np.pi])
    ax.legend(fontsize=8); ax.grid(True, alpha=0.3)

    fig.suptitle(f'Sphere locator — position & antipodality — {output_suffix}\n'
                 f'tavg window: {tavg_window_src} (t in [{tavg_t0:.2f}, {tavg_t1:.2f}], {n_tavg}/{nw} frames)',
                 fontsize=10)
...
if plot_amp_path is not None:
    ...
    # (0, 2) — time-avg radial profile (signed; both poles).  Vertical
    # markers show all four R thresholds per pole, colored to match the
    # bottom-right histogram panel (R_colors); pole is encoded by line
    # style (solid = N, dashed = S).
    ax = fig.add_subplot(gs[0, 2])
    for p in range(n_poles):
        ax.plot(rho_fit_grid[1:], vort_radial_tavg[p, 1:] / u_rms_scale,
                color=pole_colors[p], lw=1.5, label=fr'{pole_names[p]} (signed)')
        ls = '-' if p == 0 else '--'
        for k in range(n_R_types):
            ax.axvline(R_stats[p, k, 0], color=R_colors[k], ls=ls, lw=0.8, alpha=0.85)
    ax.axhline(0, color='gray', lw=0.5)
    ax.set_xscale('log')
