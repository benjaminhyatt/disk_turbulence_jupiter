...
Compares the two signed radial gradients along the fixed azimuthal slice phi = phi_CPC:

    dr(f)        = -gamma * r          (strictly negative)
    dr(omega_RW) = A * dr(nabla^2 psi_evec)(r, phi_CPC)   (signed, free to change sign)

Balance condition: -gamma * r + dr(omega_RW)(r, phi_CPC) = 0
i.e. dr(omega_RW) = gamma * r
...
### find equilibrium: where domega_dr_profile = gamma * r_profile (signed crossing) ###
...
ax.set_xlim([0, 0.5])   # focus on interior; adjust if needed
ax.legend(fontsize=9)
ax.grid(True, alpha=0.3)

# panel 2: ratio |dr(omega_RW)| / (gamma*r) vs r
ax = axes[1]
