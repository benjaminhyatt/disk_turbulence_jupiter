...
R_urms_history     = np.full(nw, np.nan)
...
for i, w in enumerate(ws):
    ...
    R_urms_history[i]  = find_threshold_crossing(rho_fit_grid, vort_radial,
                                                  omega_rms_history[i])
...
R_urms_mean,  R_urms_std,  R_urms_median  = safe_stats(R_urms_history)
...
print(f"  R_urms  (omega_rms_tavg = {omega_rms_tavg:.4f}): "
      f"mean = {R_urms_mean:.4f},  median = {R_urms_median:.4f},  "
      f"std = {R_urms_std:.4f}")
...
processed = {
    ...
    'R_urms_mean'       : R_urms_mean,   'R_urms_std'    : R_urms_std,    'R_urms_median'  : R_urms_median,
    # metadata
...
if plot_path is not None:
    ...
    ax.hist(R_urms_history[np.isfinite(R_urms_history)],   bins=bins,
            alpha=0.45, color='C5', label=f'$R_{{urms}}$ (mean={R_urms_mean:.3f})')
    ...
    ax.axvline(R_urms_mean,  color='C5', ls='--', lw=1.0)
    ax.set_xlabel(r'$R$')
    ...
    ax.plot(tw, R_urms_history,  color='C5', lw=0.7, label=r'$R_{urms}(t)$')
    ...
    ax.axhline(R_urms_mean,  color='C5', ls='--', lw=1.0, alpha=0.7)
    ax.set_xlabel('t')
    ax.set_ylabel(r'$R$')
    ...
    l_Ru  = ax.axvline(R_urms_mean,  color='C5', ls='--', lw=1.0,
                        label=f'$\\bar R_{{urms}}={R_urms_mean:.3f}$')
    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.set_xlabel(r'$\rho$')
