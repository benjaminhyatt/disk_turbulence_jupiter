...
Per non-glitch frame:
  ...
  3.  Fit a parametric profile (default: Lamb-Oseen, two parameters
      ...
      rho_fit_max(t) is by default the per-frame R_rms(t) from the
      locator.
...
Options:
    ...
    --cpc_fit_rho_max=<str>      upper rho bound for the per-frame fit;
                                 one of R_rms, R_tenth, R_fifth, R_half
                                 (per-frame history; R_urms accepted as
                                 alias for R_rms), corresponding
...
output_path = auto_or(output_arg, f"{output_prefix}_{output_suffix}_{cpc_fit_rho_max_arg}.npy")
if plot_arg.lower() in ('none', ''):
    plot_path = None
else:
    plot_path = auto_or(plot_arg, f"{output_prefix}_{output_suffix}_{cpc_fit_rho_max_arg}.png")
...
# quantity (R_rms_history / R_rms_mean is the canonical naming as of v4
# of the locator; R_urms_history / R_urms_mean is kept for backward
# compatibility with older tracking files).
_R_HIST_CANDIDATES = {
...
_R_MEAN_CANDIDATES = {
    'r_rms_mean':   ('R_rms_mean',   'R_urms_mean'),
    'r_urms_mean':  ('R_rms_mean',   'R_urms_mean'),
    'r_half_mean':  ('R_half_mean',),
    'r_fifth_mean': ('R_fifth_mean',),
    'r_tenth_mean': ('R_tenth_mean',),
}

def resolve_R_array(arg, tracking, sub_idxs, nw, default_fallback):
    """Return a per-frame array of length nw based on arg parsing."""
    arg_lo = arg.lower()
    if arg_lo in _R_HIST_CANDIDATES:
        hist_keys, mean_keys = _R_HIST_CANDIDATES[arg_lo]
        # try per-frame history first (across all alias spellings)
        for hist_key in hist_keys:
            if hist_key in tracking:
                arr = np.array(tracking[hist_key])[sub_idxs].astype(float)
                mean_val = default_fallback
                for mk in mean_keys:
                    if mk in tracking:
                        mean_val = float(tracking[mk])
                        break
                arr = np.where(np.isfinite(arr) & (arr > 0), arr, mean_val)
                return arr, 'per_frame_' + arg_lo
        # fall back to constant mean (across all alias spellings)
        for mean_key in mean_keys:
            if mean_key in tracking:
                return np.full(nw, float(tracking[mean_key])), \
                       'mean_' + arg_lo
        return np.full(nw, default_fallback), 'fallback'
    if arg_lo in _R_MEAN_CANDIDATES:
        for mean_key in _R_MEAN_CANDIDATES[arg_lo]:
            if mean_key in tracking:
                return np.full(nw, float(tracking[mean_key])), \
                       'constant_' + arg_lo
        return np.full(nw, default_fallback), 'fallback'
...
elif 'R_rms_mean' in tracking:
    init_L_default = float(tracking['R_rms_mean']) / 2
elif 'R_urms_mean' in tracking:   # back-compat
