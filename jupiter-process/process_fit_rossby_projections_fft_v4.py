...
The EVP eigenvalue scales as omega ~ gamma / k^2 (linear dispersion), so
the fractional window evp_frac_window handles the gamma-dependence naturally.

Uses projdot = projdot_c + projdot_s (real-valued, rfft).
Centroid is primary sub-bin frequency estimator; Gaussian is secondary
reference (only trusted if it falls within the centroid window).
...
def select_physical_peak(omega_centroid, peak_powers, omega_evp,
    ...
    """
    Among the top N peaks, select the one closest to omega_evp subject to:
      - |omega - omega_evp| / omega_evp <= evp_frac_window
      - power >= min_power_frac * max(peak_powers)
    Falls back to the strongest peak if no candidate passes.
    Returns the selected index into the peak arrays, and a flag indicating
    whether the EVP proximity criterion was satisfied.
    """
    ...
    if len(candidates) > 0:
        # among candidates, pick closest to EVP prediction
...
def flag_harmonics(omega_centroid, selected_idx, harmonic_frac, n_harmonics=3):
    """
    Flag peaks that are likely harmonics of the selected peak.
    Returns a boolean array of length len(omega_centroid), True = likely harmonic.
    Checks n = 2, 3, ... up to n_harmonics.
    """
...
# store full power spectrum for each mode
...
projdot_fit_params = np.zeros((nidxs, 3))   # A, omega_selected (centroid), phi
projdot_rel_err    = np.zeros((nidxs, nw))

### main loop ###
logger.info('entering FFT fitting loop')
for i, idx in enumerate(idxs_include):
    omega_evp = np.abs(evals_re[idx])   # EVP eigenvalue real part for this mode
    ...
    # flag harmonics of selected peak
    ...
    # amplitude/phase fit for all peaks, dominant uses selected peak
    ...
    for pi in range(len(omega_bin)):
        ...
        print(f"    peak {pi}: bin={omega_bin[pi]:.4f}  centroid={omega_centroid[pi]:.4f}  "
              f"power={peak_powers[pi]:.4f}  A={A_fit:.4f}{harm_str}{sel_str}")
    ...
    if sel_idx is not None and len(omega_centroid) > 0:
        ...
        print(f"  Selected fit: A={A_dom:.4f},  omega={omega_dom:.4f},  "
    ...
    if plot:
        ...
        if sel_idx is not None and len(omega_centroid) > 0:
            ax.plot(tw, projdot_fit[i,:], color='k', lw=1.2, ls='--',
                    label=f'selected fit (omega={omega_dom:.4f})')
        ax.set_xlabel('time')
        ...
        if sel_idx is not None and len(omega_bin) > 0:
            ...
            ax.plot(freqs[zmask], power_norm[zmask], color='C0', lw=1.2,
                    marker='o', ms=4, label='un-windowed bins')
            ...
            ax.axvline(omega_centroid[sel_idx], color='C3', ls='-',  lw=2.0,
                       label=f'centroid={omega_centroid[sel_idx]:.4f} [PRIMARY]')
            ax.axvline(omega_evp, color='gray', ls='--', lw=1.0,
        ...
        ax.set_title('Phase: atan2(projdot_s, projdot_c)')
...
fit_results['projdot_fit_params'] = projdot_fit_params  # A, omega_selected, phi
fit_results['projdot_rel_err']    = projdot_rel_err

# all peak data
fit_results['fft_peak_freqs']          = fft_peak_freqs_centroid  # primary
...
fit_results['fft_power_full']          = fft_power_full   # full spectrum, shape (nidxs, nfreqs)
fit_results['fft_freqs_full']          = freqs_full       # frequency axis for full spectrum
