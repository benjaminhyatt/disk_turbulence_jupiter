...
f = h5py.File('../jupiter-run/analysis_' + output_suffix + '/analysis_' + output_suffix + '_s3.h5')
...
for idx in tidxs:
    ...
    vort_max = vort['g'].max()
    mask = vort['g'] < 0.75 * vort_max
    vort['g'][mask] = 0.
    if gamma < 2e3:
        mask2 = (rnodes >= 3*L_gam)
    else:
        mask2 = (rnodes >= 2*L_gam)
    vort['g'][mask2] = 0.
    mask3 = (rnodes >= 0.6)
    vort['g'][mask3] = 0.
    vort_pile['g'] += (1/nidxs) * np.copy(vort['g'])

norm_vort_pile = d3.integ(vort_pile).evaluate()['g'][0, 0]
...
radial_norm = simpson(vort_pile_m0, r_deal[0, :]) # treating like an ordinary variable, not weighted in r
...
np.save('processed_averages_single_' + output_suffix + '.npy', processed)
