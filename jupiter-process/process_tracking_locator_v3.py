...
if plot_path is not None:
    ...
    ax.scatter([rho_fit_grid[1]], [peak_norm], color='C2', marker='*', s=90,
               zorder=5)
    ax.plot(rho_fit_grid[1:], (np.sqrt(3)/4)*(rho_fit_grid[1:])**(-1), color='C6', lw=1, label = r'$(\sqrt{3}/4)r^{-1}$')
    ...
    ax_u.axhline(np.sqrt(3)/4, color = 'C6', ls='dashdot', lw = 0.8)
    ax_u.set_yscale('log')
    ax_u.set_ylabel(r'$\bar u_\phi/\sqrt{\epsilon/\alpha}$  (right)', color='C4')
    ax_u.tick_params(axis='y', labelcolor='C4')

    # combined legend
    ax.legend(handles=[l_w, l_u, l_Rh, l_Rt], fontsize=7, loc='best')
