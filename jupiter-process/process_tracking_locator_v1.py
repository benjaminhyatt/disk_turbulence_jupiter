...
Options:
    ...
    --n_rho_fit=<int>        number of rho points (uniform spacing) [default: 96]
    ...
    --bin_width_phi=<float>      phi bin width (rad) for (phi,r) position histogram [default: 0.02]
...
### CPC-frame fit grid (uniform in rho for clean threshold detection) ###
rho_fit_grid   = np.linspace(0, rho_max_fit, n_rho_fit)
...
for i, w in enumerate(ws):
    ...
    phi_idx = int(np.floor((lon_loc % (2*np.pi)) / bin_width_phi))
    r_idx   = int(np.floor(r_loc / bin_width_r))
    if 0 <= phi_idx < n_phi_bins and 0 <= r_idx < n_r_bins:
...
if plot_path is not None:
    ...
    # panel D: time-averaged radial vorticity profile + thresholds
    ax = fig.add_subplot(gs[1, 2])
    profile = vort_radial_tavg
    peak    = profile[0]
    ax.plot(rho_fit_grid, profile, color='C2', lw=1.5, label=r'$\bar\omega(\rho)$ (time-avg)')
...
### dedicated histogram plot (polar residency map) ###
if hist_plot_path is not None:
    ...
    fig_h = plt.figure(figsize=(7.5, 7.5), constrained_layout=True)
    ax_h  = fig_h.add_subplot(1, 1, 1, projection='polar')
    phi_mesh_hist, r_mesh_hist = np.meshgrid(phi_bin_edges, r_bin_edges, indexing='ij')
    pcm = ax_h.pcolormesh(phi_mesh_hist, r_mesh_hist,
                           np.log10(position_hist + 1),
                           shading='auto', cmap='magma')
    if np.isfinite(r_mean_ok) and np.isfinite(phi_mean_ok):
        ax_h.plot([phi_mean_ok], [r_mean_ok], 'cx', ms=14, mew=2.0,
                  label=fr'$\bar r={r_mean_ok:.3f}$, $\bar\varphi={phi_mean_ok:.3f}$')
    r_outer = min(1.0, float(np.nanmax(r_locs)) * 1.2) if len(r_locs) > 0 else 1.0
    ax_h.set_ylim([0, r_outer])
    ax_h.set_title(fr'CPC position residency  —  $\gamma={gamma:.0f}$,  '
                   fr'$n_\mathrm{{frames}}={n_frames_hist}$', fontsize=10)
    ax_h.legend(loc='lower right', fontsize=8)
    cbar = fig_h.colorbar(pcm, ax=ax_h, label=r'$\log_{10}(\mathrm{count}+1)$',
                          fraction=0.046, pad=0.08)
    fig_h.suptitle(f'Position histogram  —  {output_suffix}', fontsize=11)
