...
Usage:
    process_tracking_locator_v1.py <hdf5_file> [options]
...
Options:
    ...
    --R_threshold_fifth=<float>  fraction of peak for compromise R [default: 0.2]
    --R_threshold_tenth=<float>  fraction of peak for edge-style R [default: 0.1]

    --bin_width_phi=<float>      phi bin width (rad) for (phi,r) position histogram [default: 0.05]
...
### profile helpers ###
def u_phi_from_omega(rho_grid, omega_radial):
    """Axisymmetric (m=0) u_phi(rho) from the alpha-averaged vorticity
    via omega = (1/rho) d/drho (rho u_phi); inverts to
        u_phi(rho) = (1/rho) * integral_0^rho rho' * omega(rho') drho'
    Regularity at rho=0 gives u_phi(0) = 0. The integration is
    trapezoidal on whatever grid is provided (uniform or log)."""
    from scipy.integrate import cumulative_trapezoid
    integrand = omega_radial * rho_grid
    cum_int   = cumulative_trapezoid(integrand, rho_grid, initial=0.0)
    u_phi     = np.zeros_like(rho_grid)
    nz        = rho_grid > 0
    u_phi[nz] = cum_int[nz] / rho_grid[nz]
    return u_phi
...
### load EN(t) over the analysis window, get omega_rms(t) ###
# EN is the spatial integral over the unit disk (area = pi), so the
# rms vorticity is sqrt(<omega^2>) = sqrt(EN / pi).
EN_window         = np.asarray(f['tasks/EN'][ws_start:ws_end+1]).reshape(-1)
omega_rms_history = np.sqrt(EN_window / np.pi)
omega_rms_tavg    = float(np.mean(omega_rms_history))
print(f"omega_rms_tavg = {omega_rms_tavg:.4f}  "
      f"(min={omega_rms_history.min():.4f}, max={omega_rms_history.max():.4f})")
...
R_fifth_history    = np.full(nw, np.nan)
R_tenth_history    = np.full(nw, np.nan)
R_rms_history     = np.full(nw, np.nan)
...
for i, w in enumerate(ws):
    ...
    R_fifth_history[i] = find_threshold_crossing(rho_fit_grid, vort_radial,
                                                  R_thresh_fifth * omega_0)
    R_tenth_history[i] = find_threshold_crossing(rho_fit_grid, vort_radial,
                                                  R_thresh_tenth * omega_0)
    R_rms_history[i]  = find_threshold_crossing(rho_fit_grid, vort_radial,
                                                  omega_rms_history[i])
...
u_phi_tavg       = u_phi_from_omega(rho_fit_grid, vort_radial_tavg)
...
R_fifth_mean, R_fifth_std, R_fifth_median = safe_stats(R_fifth_history)
R_tenth_mean, R_tenth_std, R_tenth_median = safe_stats(R_tenth_history)
R_rms_mean,  R_rms_std,  R_rms_median  = safe_stats(R_rms_history)
...
print(f"  R_half  (peak fraction = {R_thresh_half}):  "
      ...
      f"std = {R_half_std:.4f}")
print(f"  R_fifth (peak fraction = {R_thresh_fifth}): "
      f"mean = {R_fifth_mean:.4f},  median = {R_fifth_median:.4f},  "
      f"std = {R_fifth_std:.4f}")
print(f"  R_tenth (peak fraction = {R_thresh_tenth}): "
      f"mean = {R_tenth_mean:.4f},  median = {R_tenth_median:.4f},  "
      f"std = {R_tenth_std:.4f}")
print(f"  R_rms   (omega_rms_tavg = {omega_rms_tavg:.4f}): "
      f"mean = {R_rms_mean:.4f},  median = {R_rms_median:.4f},  "
      f"std = {R_rms_std:.4f}")
...
processed = {
    ...
    'R_fifth_history'   : R_fifth_history,
    'R_tenth_history'   : R_tenth_history,
    'R_rms_history'    : R_rms_history,
    'R_urms_history'   : R_rms_history,   # back-compat alias
    'omega_peak_history': omega_peak_history,
    'omega_rms_history' : omega_rms_history,
    'omega_rms_tavg'    : omega_rms_tavg,
    'vort_radial_tavg'  : vort_radial_tavg,
    'u_phi_tavg'        : u_phi_tavg,
    'n_valid_for_avg'   : n_valid_for_avg,
    'R_thresh_half'     : R_thresh_half,
    'R_thresh_fifth'    : R_thresh_fifth,
    'R_thresh_tenth'    : R_thresh_tenth,
    'R_half_mean'       : R_half_mean,   'R_half_std'    : R_half_std,    'R_half_median'  : R_half_median,
    'R_fifth_mean'      : R_fifth_mean,  'R_fifth_std'   : R_fifth_std,   'R_fifth_median' : R_fifth_median,
    'R_tenth_mean'      : R_tenth_mean,  'R_tenth_std'   : R_tenth_std,   'R_tenth_median' : R_tenth_median,
    'R_rms_mean'       : R_rms_mean,   'R_rms_std'    : R_rms_std,    'R_rms_median'  : R_rms_median,
    # back-compat aliases
    'R_urms_mean'      : R_rms_mean,   'R_urms_std'   : R_rms_std,    'R_urms_median' : R_rms_median,
    # metadata
...
if plot_path is not None:
    ...
    # panel B: R histograms (all four thresholds)
    ax = fig.add_subplot(gs[0, 2])
    bins = np.linspace(0, rho_max_fit, 40)
    ax.hist(R_half_history[np.isfinite(R_half_history)],   bins=bins,
            alpha=0.45, color='C0', label=f'$R_{{1/2}}$ (mean={R_half_mean:.3f})')
    ax.hist(R_fifth_history[np.isfinite(R_fifth_history)], bins=bins,
            alpha=0.45, color='C1', label=f'$R_{{1/5}}$ (mean={R_fifth_mean:.3f})')
    ax.hist(R_tenth_history[np.isfinite(R_tenth_history)], bins=bins,
            alpha=0.45, color='C3', label=f'$R_{{1/10}}$ (mean={R_tenth_mean:.3f})')
    ax.hist(R_rms_history[np.isfinite(R_rms_history)],   bins=bins,
            alpha=0.45, color='C5', label=f'$R_\\mathrm{{rms}}$ (mean={R_rms_mean:.3f})')
    ax.axvline(R_half_mean,  color='C0', ls='--', lw=1.0)
    ax.axvline(R_fifth_mean, color='C1', ls='--', lw=1.0)
    ax.axvline(R_tenth_mean, color='C3', ls='--', lw=1.0)
    ax.axvline(R_rms_mean,  color='C5', ls='--', lw=1.0)
    ax.set_xlabel(r'$R$')
    ...
    # panel C: R(t) — all four thresholds
    ax = fig.add_subplot(gs[1, 0:2])
    ax.plot(tw, R_half_history,  color='C0', lw=0.7, label=r'$R_{1/2}(t)$')
    ax.plot(tw, R_fifth_history, color='C1', lw=0.7, label=r'$R_{1/5}(t)$')
    ax.plot(tw, R_tenth_history, color='C3', lw=0.7, label=r'$R_{1/10}(t)$')
    ax.plot(tw, R_rms_history,  color='C5', lw=0.7, label=r'$R_\mathrm{rms}(t)$')
    ax.axhline(R_half_mean,  color='C0', ls='--', lw=1.0, alpha=0.7)
    ax.axhline(R_fifth_mean, color='C1', ls='--', lw=1.0, alpha=0.7)
    ax.axhline(R_tenth_mean, color='C3', ls='--', lw=1.0, alpha=0.7)
    ax.axhline(R_rms_mean,  color='C5', ls='--', lw=1.0, alpha=0.7)
    ax.set_xlabel('t')
    ax.set_ylabel(r'$R$')
    ...
    # panel D: time-avg profiles (log-log) — vorticity + u_phi on twin y,
    # both normalized by sqrt(eps/alpha)
    ax = fig.add_subplot(gs[1, 2])
    u_rms_scale  = float(np.sqrt(eps / alpha))
    profile      = vort_radial_tavg
    peak         = profile[0]                  # rho=0
    profile_norm = profile     / u_rms_scale
    u_phi_norm   = u_phi_tavg  / u_rms_scale
    peak_norm    = peak        / u_rms_scale

    # vorticity on left y-axis (green)
    l_w, = ax.plot(rho_fit_grid[1:], profile_norm[1:], color='C2', lw=1.5,
                    label=r'$\bar\omega(\rho)$')
    ax.scatter([rho_fit_grid[1]], [peak_norm], color='C2', marker='*', s=90,
               zorder=5)
    # threshold horizontals (fractions of peak, plus background omega_rms)
    ax.axhline(R_thresh_half  * peak_norm,         color='C0', ls=':', lw=0.8)
    ax.axhline(R_thresh_fifth * peak_norm,         color='C1', ls=':', lw=0.8)
    ax.axhline(R_thresh_tenth * peak_norm,         color='C3', ls=':', lw=0.8)
    ax.axhline(omega_rms_tavg / u_rms_scale,       color='C5', ls=':', lw=0.8)
    # vertical R markers
    l_Rh  = ax.axvline(R_half_mean,  color='C0', ls='--', lw=1.0,
                        label=f'$\\bar R_{{1/2}}={R_half_mean:.3f}$')
    l_R5  = ax.axvline(R_fifth_mean, color='C1', ls='--', lw=1.0,
                        label=f'$\\bar R_{{1/5}}={R_fifth_mean:.3f}$')
    l_Rt  = ax.axvline(R_tenth_mean, color='C3', ls='--', lw=1.0,
                        label=f'$\\bar R_{{1/10}}={R_tenth_mean:.3f}$')
    l_Ru  = ax.axvline(R_rms_mean,  color='C5', ls='--', lw=1.0,
                        label=f'$\\bar R_\\mathrm{{rms}}={R_rms_mean:.3f}$')
    ax.set_xscale('log')
    ax.set_yscale('log')
    ax.set_xlabel(r'$\rho$')
    ax.set_ylabel(r'$\bar\omega/\sqrt{\epsilon/\alpha}$  (left)', color='C2')
    ax.tick_params(axis='y', labelcolor='C2')
    ax.set_title(f'Time-avg profiles  (peak $\\bar\\omega_0/\\sqrt{{\\epsilon/\\alpha}}={peak_norm:.2f}$)')
    ax.grid(True, which='both', alpha=0.3)

    # u_phi on twin y-axis (purple), log scale
    ax_u = ax.twinx()
    l_u, = ax_u.plot(rho_fit_grid[1:], u_phi_norm[1:], color='C4', lw=1.5,
                      label=r'$\bar u_\phi(\rho)$')
    ax_u.set_yscale('log')
    ax_u.set_ylabel(r'$\bar u_\phi/\sqrt{\epsilon/\alpha}$  (right)', color='C4')
    ax_u.tick_params(axis='y', labelcolor='C4')

    # combined legend
    ax.legend(handles=[l_w, l_u, l_Rh, l_R5, l_Rt, l_Ru],
...
if hist_plot_path is not None:
    ...
    # phi mean intentionally not computed: distribution is expected to be
    # roughly uniform around the pole, so the circular mean has no
    ...
    # panel: 1D phi-histogram (no mean line — distribution expected ~uniform)
    ...
    # dashed ring at r = r_mean (meaningful), no phi-specific marker
    if np.isfinite(r_mean_ok):
        phi_ring = np.linspace(0, 2*np.pi, 361)
        r_ring   = np.full_like(phi_ring, r_mean_ok)
        ax3.plot(phi_ring, r_ring, color='cyan', ls='--', lw=1.5,
                 label=fr'$\bar r={r_mean_ok:.3f}$')
