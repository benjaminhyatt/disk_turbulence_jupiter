...
Options:    
    --output=<str>          prefix in the name of the output file [default: processed_profiles_filtered_nom1]
"""
...
u_nom1 = dist.VectorField(coords, bases=disk)
...
for k, w in enumerate(ws_tracked):
    ...
    u_nom1.change_scales(dealias)
    u_nom1['c'] *= 0.
    u_nom1['c'][:, :4, :] += np.copy(u['c'][:, :4, :])
    u_nom1['c'][:, 6:, :] += np.copy(u['c'][:, 6:, :])
