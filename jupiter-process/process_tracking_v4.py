...
Options:
    ...
    --max_jump_phi=<float>  max allowed jump in phi (rad) between frames before flagging as glitch [default: 1.8]
...
for i, w in enumerate(ws):
    ...
    if extract_vel and not glitch:
        ...
        if subtract_rw:
            ...
            # RW velocity from psi_RW: u_phi_RW = dr(psi_RW), u_r_RW = -1/r * dphi(psi_RW)
            # compute using Dedalus for accuracy
            psi_rw_field.change_scales(dealias)
            psi_rw_field['g'] = psi_rw_g
            uphi_rw = # (d3.radial(d3.grad(psi_rw_field))).evaluate()
            ur_rw   = # (-d3.azimuthal(d3.grad(psi_rw_field)) / d3.radius).evaluate()
            uphi_rw.change_scales(dealias)
            ur_rw.change_scales(dealias)

            # subtract RW from lab frame velocity
            ur_sub   = ur_g   - ur_rw['g']
            uphi_sub = uphi_g - uphi_rw['g']
