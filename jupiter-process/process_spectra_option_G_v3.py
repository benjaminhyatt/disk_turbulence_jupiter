...
Usage:
    process_spectra_optionG.py <file>... [options]
...
Options:
    ...
    --stride=<int>              Subsample writes by this factor in the "middle" of the time range; the early-transient and late-steady windows are still processed at every write [default: 10]
    --dense_early_until=<value>  Up to this simulation time, process every write (no striding); resolves the fast initial transient [default: 2.0]
    --dense_late_from=<value>   From this simulation time onward, process every write (no striding); resolves the steady-state.  "auto" means t_steady_range seconds prior to t_out_end [default: auto]
...
dense_early_until     = float(args['--dense_early_until'])
dense_late_from_arg   = args['--dense_late_from']
make_new              = eval(args['--make_new'])
steady_only           = eval(args['--steady_only'])
# t_out_start, t_out_end, t_steady_range, and dense_late_from are resolved
# below, after the HDF5 file's time array is loaded.

output_prefix = 'processed_spectra_option_G_v3'
...
# Full range of writes covering [t_out_start, t_out_end].
ws_all = np.arange(np.where(t <= t_out_start)[0][-1],
                    np.where(t >= t_out_end)[0][0] + 1)

# Resolve the late-dense start time.
if str(dense_late_from_arg).lower() == 'auto':
    dense_late_from = t_out_end - t_steady_range #min(t_steady_range, 10.0)
else:
    dense_late_from = float(dense_late_from_arg)

# Apply the cluster sampling: every write in the early-dense window, every
# write in the late-dense window, every `stride`-th write in the middle.
t_at_ws_all   = t[ws_all]
early_mask    = t_at_ws_all <= dense_early_until
late_mask     = t_at_ws_all >= dense_late_from
middle_mask   = (~early_mask) & (~late_mask)
middle_idx    = np.where(middle_mask)[0]
if stride > 1 and len(middle_idx) > 0:
    middle_keep = middle_idx[::stride]
else:
    middle_keep = middle_idx
keep_mask = early_mask | late_mask
keep_mask[middle_keep] = True
ws = ws_all[keep_mask]
nw = len(ws)
tw = t[ws]
if rank == 0:
    n_early  = int(np.sum(early_mask))
    n_middle = int(np.sum(keep_mask & middle_mask))
    n_late   = int(np.sum(late_mask))
    logger.info("Sampling: %d writes total  (early=%d, middle=%d strided-by-%d, late=%d)",
                nw, n_early, n_middle, stride, n_late)
    logger.info("Dense windows: [%.3f, %.3f] and [%.3f, %.3f]",
                t_out_start, dense_early_until, dense_late_from, t_out_end)
