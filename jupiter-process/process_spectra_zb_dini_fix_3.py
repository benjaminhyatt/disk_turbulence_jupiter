...
Usage:
    process_spectra_zb_dini_fix.py [options]
...
def dini_weights(m, dini_zsm):
    ...
    return (2 * dini_zsm**2) / ((H**2 + dini_zsm**2 - m**2) * sp.jv(m, dini_zsm)**2)
...
def ke_m(m, Nr, psimc, psims, dini_zs):
    ...
    return jzsm**2 * (psimc**2 + psims**2) * (2 / Nphi) * dini_weight_m# 2 / Nphi # dini weights?
...
def en_m(m, Nr, psimc, psims, dini_zs):
    ...
    return jzsm**4 * (psimc**2 + psims**2) * (2 / Nphi) * dini_weight_m # 2 / Nphi # dini weights?
...
if rank == 0:
    ...
    if use_forcing:
        ...
        np.save('processed_spectra_zb_dini_fix_3_' + output_suffix + '.npy', processed)
else:
