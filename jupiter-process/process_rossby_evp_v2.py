...
Options:
    ...
    --u0bg=<bool>       [default: True] (False to not specify a background u0 profile, in which case, <file> should be an analysis file)
"""
import numpy as np
import h5py
...
output_prefix += '_u0bg_{:d}'.format(u0bg)
...
gamma_vals = np.array((0, 30, 85, 240, 400, 675, 920, 1200, 1920, 2372, 2500, 3200))
...
def evp_dense(Nphi, Nr, u0filename, inviscid, params):
    ...
    if inviscid:
        m, gamma, save_psi, save_vort, u0bg = params
    else:
        m, gamma, alpha, nu, save_psi, save_vort, u0bg = params
    ...
    elif inviscid and (not u0bg):
        problem = d3.EVP([psi, tau_psi, tau_psi2], eigenvalue=om, namespace=locals())
        problem.add_equation("dt(lap(psi)) + skew(grad(psi))@grad(f) + lift(tau_psi2, -1) + tau_psi = 0")
    else: 
        problem = d3.EVP([psi, tau_psi, tau_psi2, tau_psi3], eigenvalue=om, namespace=locals())
        problem.add_equation("dt(lap(psi)) + alpha*lap(psi) - nu*lap(lap(psi)) + skew(grad(psi))@grad(f) + lift(tau_psi2, -1) + lift(tau_psi3, -2) + tau_psi = 0")
        problem.add_equation("azimuthal(radial(stress(u_psi(psi))(r=1))) = 0")
...
if inviscid:
    params = (m, gamma, save_psi, save_vort, u0bg)
    params_hi = (m, gamma, False, False, u0bg)
else:
    params = (m, gamma, alpha, nu, save_psi, save_vort, u0bg)
    params_hi = (m, gamma, alpha, nu, False, False, u0bg)
