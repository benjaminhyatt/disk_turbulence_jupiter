...
Conventions (Ben's, from force_balance_hypothesis.pdf):
    ...
    u_phi   = d_r psi
    u_r     = -(1/r) d_phi psi
i.e. u = +skew(grad(psi)) in Dedalus operators (since d3.Skew on a
PolarCoordinates('phi','r') vector field acts as
[v_phi, v_r] -> [v_r, -v_phi]; applied to grad(psi) = [(1/r) d_phi psi,
d_r psi] this gives [d_r psi, -(1/r) d_phi psi] = [u_phi, u_r]).
...
tracking = None
v4_to_track_idx = None   # (nw_v4,) ints; -1 where no tracking match
if tracking_file_arg.lower() != 'auto':
    tracking = np.load(tracking_file_arg, allow_pickle=True)[()]
    print("keys in tracking", list(tracking.keys()))
    # Build a per-v4-frame mapping into the tracking arrays by matching
    # write-indices (ws).  Robust to tracking having been run over a
    # larger time interval than v4 (or vice versa): we only use frames
    # that appear in both.
    if 'ws' not in tracking:
        print("  WARNING: tracking has no 'ws' field; cannot map v4 ↔ "
              "tracking by write-index. Falling back to positional "
              "indexing (will error if lengths disagree).")
    else:
        ws_track_arr = np.asarray(tracking['ws']).astype(int)
        ws_track_to_idx = {int(w): j for j, w in enumerate(ws_track_arr)}
        v4_to_track_idx = np.array(
            [ws_track_to_idx.get(int(w), -1) for w in ws_v4], dtype=int)
        n_match = int(np.sum(v4_to_track_idx >= 0))
        print(f"tracking↔v4 mapping by ws: {n_match}/{nw_v4} v4 frames "
              f"matched (tracking nw={len(ws_track_arr)})")
        if n_match == 0:
            raise ValueError(
                "No v4 frames matched any tracking frame by ws; the two "
                ".npy files likely came from different runs.")
        if n_match < nw_v4:
            n_miss = nw_v4 - n_match
            print(f"  NOTE: {n_miss} v4 frames have no tracking match — "
                  f"R values at those frames will fall back to the mean "
                  f"of matched frames.")
...
# Note: 'r_rms' and 'r_urms' are accepted as aliases for the same
# quantity (R_rms is the canonical naming as of v4 of the locator;
# R_urms is kept for backward compatibility with older tracking files).
_HIST_CANDIDATES = {
    'r_rms':   (('R_rms_history',   'R_urms_history'),
                ('R_rms_mean',      'R_urms_mean')),
    'r_urms':  (('R_rms_history',   'R_urms_history'),
                ('R_rms_mean',      'R_urms_mean')),
...
def parse_integrate_rho_max(arg, nw, L_fit_sub, tracking, sub_idxs_v4):
    ...
    if arg_lo.endswith('l'):
        try:
            ...
            pass
    # per-frame R_* (try v4 first, then tracking; try all alias spellings)
    if arg_lo in _HIST_CANDIDATES:
        hist_keys, mean_keys = _HIST_CANDIDATES[arg_lo]
        # try per-frame history (across all alias spellings, v4 first then tracking)
        for hist_key in hist_keys:
            if hist_key in v4:
                arr_full = np.asarray(v4[hist_key], dtype=float)
                arr = arr_full[sub_idxs_v4]
                fallback = float(np.nanmean(
                    arr[np.isfinite(arr) & (arr > 0)]))
                arr = np.where(np.isfinite(arr) & (arr > 0), arr, fallback)
                return arr, f'per_frame_{arg_lo}(v4:{hist_key})'
            if tracking is not None and hist_key in tracking:
                arr_full = np.asarray(tracking[hist_key], dtype=float)
                # map tracking → v4 frames via write-index match (preferred),
                # else fall back to direct indexing if lengths agree
                if v4_to_track_idx is not None:
                    arr_v4 = np.full(nw_v4, np.nan)
                    valid_mask = v4_to_track_idx >= 0
                    arr_v4[valid_mask] = arr_full[v4_to_track_idx[valid_mask]]
                    arr = arr_v4[sub_idxs_v4]
                    src_tag = f'tracking:{hist_key}@ws'
                elif len(arr_full) == nw_v4:
                    arr = arr_full[sub_idxs_v4]
                    src_tag = f'tracking:{hist_key}'
                else:
                    raise ValueError(
                        f"Length mismatch between tracking '{hist_key}' "
                        f"({len(arr_full)}) and v4 frames ({nw_v4}), and "
                        f"no 'ws' field in tracking to match on. Pass a "
                        f"--tracking_file whose 'ws' overlaps the v4 .npy.")
                # fallback for any NaN / non-positive entries (incl.
                # unmatched frames) → mean of valid matched entries
                good = np.isfinite(arr) & (arr > 0)
                if good.any():
                    fallback = float(np.nanmean(arr[good]))
                    arr = np.where(good, arr, fallback)
                return arr, f'per_frame_{arg_lo}({src_tag})'
        # fall back to constant mean
        for mean_key in mean_keys:
            if mean_key in v4:
                return np.full(nw, float(v4[mean_key])), \
                       f'mean_{arg_lo}(v4:{mean_key})'
            if tracking is not None and mean_key in tracking:
                return np.full(nw, float(tracking[mean_key])), \
                       f'mean_{arg_lo}(tracking:{mean_key})'
        raise ValueError(
            f"Could not resolve --integrate_rho_max={arg}; no matching "
            f"arrays (tried {hist_keys} and {mean_keys}) in v4 .npy "
            f"or tracking.")
    # mean_* keys (literal lookup, case-insensitive)
...
### For Ben's convention (ω = +∇²ψ, u_φ = ∂_r ψ, u_r = −(1/r) ∂_φ ψ): ###
###   grad(ψ)         = [(1/r) ∂_φ ψ, ∂_r ψ]   (Dedalus order phi,r)  ###
###   skew(grad(ψ))   = [∂_r ψ, −(1/r) ∂_φ ψ]  (per d3.skew test:     ###
###                                              [v_phi,v_r] ↦ [v_r,  ###
###                                              −v_phi])             ###
###   => u = +skew(grad(ψ)) gives u_φ = ∂_r ψ, u_r = −(1/r) ∂_φ ψ ✓   ###
...
def eigenmode_velocity_components(psi_arr_real):
    ...
    components via u = +skew(grad(psi)).  See block comment above for the
