"""
RW amplitude via CPC-template joint LSQ (Phase A.).

Replaces a previous ad hoc per-frame amplitude scaling with a principled joint
...
Usage:
    process_rw_amplitude_subtracted_v1.py <hdf5_file> <evp_file> <tracking_file> [options]
...
Options:
    ...
    --output_prefix=<str>        [default: processed_rw_amplitude_subtracted_v1]
...
for i in range(nw):
    ...
    Delta = psi_T_norm2 - np.sum(C_vec * D_vec)  # may be complex; ideally ~ real
    num   = psi_T_psi  - np.sum(C_vec * b_vec)
...
if plot_path is not None:
    ...
    if snapshot_vort is not None:
        ...
        pcm = ax.pcolormesh(phi_mesh, r_mesh, snapshot_vort,
                             ...
                             vmin=-vmax, vmax=vmax)
        mid_phi = float(phi_sub[mid_idx]); mid_r = float(r_sub[mid_idx])
        ax.plot([mid_phi], [mid_r], 'kx', ms=10, mew=2)
