...
Options:
    ...
    --t_steady_frac=<float>     Fraction of the time range to use for the
                                ...
                                [default: 0.5]
    --strict_cartesian_band     Use the strict [k_f*pi, (k_f+1)*pi] band
                                (no half-bin padding).  Default is the
                                wider, disk-basis-spread-aware band.
...
def parse_params_from_filename(filename):
    ...
    # Note: actual band edges (with or without half-bin padding) are computed
    # later in compute_diagnostics, using `band_pad`.
    if out['kf'] is not None:
        out['k_band_low_strict']  = out['kf'] * np.pi
        out['k_band_high_strict'] = (out['kf'] + 1) * np.pi
    return out
...
def compute_diagnostics(data, option, params, t_steady_frac=0.5,
                        strict_cartesian_band=False):
    """Compute all derived diagnostic quantities for a single processed file.

    Band partitioning: by default the middle ("band") region is widened by
    half a bin-width (~pi/2) on each side, so that the disk-basis spread of
    the Cartesian forcing shell is captured in the middle band rather than
    split with the adjacent low/high regions.  Pass strict_cartesian_band=True
    to use the narrow [k_f*pi, (k_f+1)*pi] convention instead.
    """
    ...
    # Build band masks.  With strict_cartesian_band, use the IVP forcing's
    # strict Cartesian shell [k_f*pi, (k_f+1)*pi].  Otherwise pad -- there is
    # spread across the bins adjacent to the strict band, so widening captures
    # the physical "energy at the forcing scale" region.
    if params.get('k_band_low_strict') is not None:
        pad = 0.0 if strict_cartesian_band else 2.0 * np.pi
        k_low_edge  = params['k_band_low_strict']  - pad
        k_high_edge = params['k_band_high_strict'] + pad
        m_low, m_band, m_high = band_masks(bin_centers, k_low_edge, k_high_edge)
    else:
    ...
    if np.sum(valid) >= 4:
        ...
        try:
            ...
            # Identify which component is fast / slow (by rate magnitude)
            # and pair each rate with its matching amplitude.
            if abs(popt2[1]) >= abs(popt2[3]):
                A_fast, r_2exp_fast = float(popt2[0]), float(abs(popt2[1]))
                A_slow, r_2exp_slow = float(popt2[2]), float(abs(popt2[3]))
            else:
                A_fast, r_2exp_fast = float(popt2[2]), float(abs(popt2[3]))
                A_slow, r_2exp_slow = float(popt2[0]), float(abs(popt2[1]))
            ...
            A_fast, A_slow = float('nan'), float('nan')
    else:
        r_1exp = r_2exp_fast = r_2exp_slow = AIC_1exp = AIC_2exp = float('nan')
        A_fast = A_slow = float('nan')
        popt1 = popt2 = None
        t_fit = y_fit = None
        t0 = None

    # Derived "retention after fast transient" and per-stage loss fractions.
    # Interpretation: the 2-exp fit decomposes  KE(t) - KE_steady  into a
    # fast-decaying component (amplitude A_fast at t=t0) and a slow-decaying
    # component (amplitude A_slow at t=t0).  After the fast transient (say
    # t = a few / r_fast), the fast piece has decayed away and the system
    # holds approximately  KE_steady + A_slow.  All of A_slow eventually
    # decays too as t -> infinity.  So we get:
    #   fast loss fraction = A_fast / KE_initial
    #   slow loss fraction = A_slow / KE_initial
    #   R_after_fast       = (KE_steady + A_slow) / KE_initial = R + A_slow/KE_initial
    if (not np.isnan(A_fast)) and (not np.isnan(A_slow)) and KE_initial > 1e-30:
        R_after_fast       = float((KE_steady + A_slow) / KE_initial)
        fast_loss_fraction = float(A_fast / KE_initial)
        slow_loss_fraction = float(A_slow / KE_initial)
    else:
        R_after_fast       = float('nan')
        fast_loss_fraction = float('nan')
        slow_loss_fraction = float('nan')
    ...
    return dict(
        ...
        R_after_fast=R_after_fast,
        fast_loss_fraction=fast_loss_fraction,
        slow_loss_fraction=slow_loss_fraction,
...
def make_diagnostics_figure(datasets, options, params, out_stem,
    ...
    for data, opt in zip(datasets, options):
        d = compute_diagnostics(data, opt, params, t_steady_frac=t_steady_frac,
                                 strict_cartesian_band=strict_cartesian_band)
        diags.append((opt, d))
    ...
    if n_methods == 1:
        ...
        summary_lines = [
            f"R (final)      = {d['R']:.3g}",
            f"R (after fast) = {d['R_after_fast']:.3g}",
            f"fast loss frac = {d['fast_loss_fraction']:.3g}",
            f"slow loss frac = {d['slow_loss_fraction']:.3g}",
...
def write_json(diags, params, options, out_path):
    ...
    for (opt, d) in diags:
        out['methods'][opt] = dict(
            ...
            R=d['R'],
            R_after_fast=d['R_after_fast'],
            A_fast=d['A_fast'],
            A_slow=d['A_slow'],
            fast_loss_fraction=d['fast_loss_fraction'],
            slow_loss_fraction=d['slow_loss_fraction'],
            f_nu=d['f_nu'],
            f_alpha=d['f_alpha'],
...
def main():
    ...
    strict_band   = bool(args.get('--strict_cartesian_band'))
    ...
    fig, diags = make_diagnostics_figure(
        ...
        strict_cartesian_band=strict_band,
    )
    if not no_figure:
