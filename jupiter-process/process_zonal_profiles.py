...
disk_vfield = dist.VectorField(coords, name = 'disk_vf', bases = disk)
...
# take derivatives, evaluate at edge
uphi_edge = np.zeros((nwritesf, Nphi))
dr_uphi_edge = np.zeros((nwritesf, Nphi))
dr2_uphi_edge = np.zeros((nwritesf, Nphi))
vort_edge = np.zeros((nwritesf, Nphi))
dr_vort_edge = np.zeros((nwritesf, Nphi))
...
for w in range(nwritesf):
    ...
    disk_vfield.change_scales(1)
    disk_vfield['g'] = dset_u[w, :, :, :] # u
    dr_uphi_edge_field = (ephi@(er@d3.grad(disk_vfield)))(r = 1).evaluate()
    dr_uphi_edge_field.change_scales(1)
    dr_uphi_edge[w] = dr_uphi_edge_field['g'][:, 0]

    disk_vfield.change_scales(1)
    disk_vfield['g'] = dset_u[w, :, :, :] # u
    dr2_uphi_edge_field = (ephi@(er@d3.grad(er@d3.grad(disk_vfield))))(r = 1).evaluate()
    dr2_uphi_edge_field.change_scales(1)
    dr2_uphi_edge[w] = dr2_uphi_edge_field['g'][:, 0]
    ...
    disk_field.change_scales(1)
    disk_field['g'] = dset_vort[w, :, :] # vort
    dr_vort_edge_field = (er@d3.grad(disk_field))(r = 1).evaluate()
    dr_vort_edge_field.change_scales(1)
    dr_vort_edge[w] = dr_vort_edge_field['g'][:, 0]
...
subtasks_out = {}
subtasks_out['profiles_u'] = ['uphi_edge', 'uphi_edge_tavg', 'dr_uphi_edge', 'dr_uphi_edge_tavg', 'dr2_uphi_edge', 'dr2_uphi_edge_tavg'] 
subtasks_out['profiles_vort'] = ['vort_edge', 'vort_edge_tavg', 'dr_vort_edge', 'dr_vort_edge_tavg']

labels_out = {}
labels_out['profiles_u'] = [r'$u_{\phi}(r = 1)$', r'$\langle u_{\phi}(r = 1) \rangle$', r'$\partial_r u_{\phi}(r = 1)$', r'$\langle \partial_r u_{\phi}(r = 1) \rangle$', r'$\partial_r^2 u_{\phi}(r = 1)$', r'$\langle \partial_r^2 u_{\phi}(r = 1) \rangle$']
labels_out['profiles_vort'] = [r'$\omega (r = 1)$', r'$\langle \omega (r = 1) \rangle$', r'$\partial_r \omega (r = 1)$', r'$\langle \partial_r \omega (r = 1) \rangle$']
...
dr2_uphi_edge_tavg = np.mean(dr2_uphi_edge[tstartidx:, :], axis = 0)
vort_edge_tavg = np.mean(vort_edge[tstartidx:, :], axis = 0)
dr_vort_edge_tavg = np.mean(dr_vort_edge[tstartidx:, :], axis = 0)


ymin_u = np.min(uphi_edge_tavg)
ymin_u2 = np.min(dr_uphi_edge_tavg)
ymin_u3 = np.min(dr2_uphi_edge_tavg)
ymin_u = np.min([ymin_u, ymin_u2, ymin_u3])
ymax_u = np.max(uphi_edge_tavg)
ymax_u2 = np.max(dr_uphi_edge_tavg)
ymax_u3 = np.max(dr2_uphi_edge_tavg)
ymax_u = np.max([ymax_u, ymax_u2, ymax_u3])

ymin_vort = np.min(vort_edge_tavg)
ymin_vort2 = np.min(dr_vort_edge_tavg)
ymin_vort = np.min([ymin_vort, ymin_vort2])
ymax_vort = np.max(vort_edge_tavg)
ymax_vort2 = np.max(dr_vort_edge_tavg)
ymax_vort = np.max([ymax_vort, ymax_vort2])
...
for w in range(nwritesf):
    ...
    for taskout in tasks_out:
        ...
        if taskout == 'profiles_u':
            ...
            processed[idx + w][taskout]['data_dr2_uphi_edge'] = dr2_uphi_edge[w]
            processed[idx + w][taskout]['data_dr2_uphi_edge_tavg'] = dr2_uphi_edge_tavg
        if taskout == 'profiles_vort':
            ...
            processed[idx + w][taskout]['data_dr_vort_edge'] = dr_vort_edge[w]
            processed[idx + w][taskout]['data_dr_vort_edge_tavg'] = dr_vort_edge_tavg
...
print("Saving output")
np.save('processed_zonal_profiles_' + output_suffix + '.npy', processed)
