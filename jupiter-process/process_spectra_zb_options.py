...
Usage:
    process_spectra_zb_options.py <file>... [options]
...
output_prefix = 'processed_spectra_zb'
if dini: 
    output_prefix += '_dini'
else:
    output_prefix += '_std'
...
def bin_spectra_total(data, Nbins, masks, case):
    nspec = []
    for b in range(Nbins):
        mask = masks[b]
...
if dini:
    ...
    _, _, _, _, zonal_masks = define_bins(dini_zs, 'zonal')
    _, _, _, _, non_zonal_masks = define_bins(dini_zs, 'non_zonal')
else:
    Nbins, centers, edges, counts, masks = define_bins(std_zs, None)
    _, _, _, _, zonal_masks = define_bins(std_zs, 'zonal')
    _, _, _, _, non_zonal_masks = define_bins(std_zs, 'non_zonal')
...
fluxBn_zonal = np.zeros((nw, Nbins))
keBn_nz = np.zeros((nw, Nbins))
enBn_nz = np.zeros((nw, Nbins))
fluxBn_nz = np.zeros((nw, Nbins))
...
for i, w in enumerate(ws):
    ...
    if rank == 0:
        ...
        keBn_zonal[i, :] = bin_spectra(keB[i, :, :], bin_widths, Nbins, zonal_masks, 'zonal')
        enBn_zonal[i, :] = bin_spectra(enB[i, :, :], bin_widths, Nbins, zonal_masks, 'zonal')
        fluxBn_zonal[i, :] = flux_k(keBn_zonal[i, :], lambda_k(keBn_zonal[i, :], centers), centers)
        
        keBn_nz[i, :] = bin_spectra(keB[i, :, :], bin_widths, Nbins, non_zonal_masks, 'non_zonal')
        enBn_nz[i, :] = bin_spectra(enB[i, :, :], bin_widths, Nbins, non_zonal_masks, 'non_zonal')
        fluxBn_nz[i, :] = flux_k(keBn_nz[i, :], lambda_k(keBn_nz[i, :], centers), centers)
...
if rank == 0:
    ...
    if not steady_only:
        ...
        processed['fluxBn_zonal'] = fluxBn_zonal
        processed['keBn_nz'] = keBn_nz
        processed['enBn_nz'] = enBn_nz
        processed['fluxBn_nz'] = fluxBn_nz
    ...
    if len(ws) > 1:
        ...
        fluxBn_zonal_tavg = np.mean(fluxBn_zonal[tavg_start_idx:tavg_end_idx, :], axis = 0)
        processed['keBn_zonal_tavg'] = keBn_zonal_tavg
        processed['enBn_zonal_tavg'] = enBn_zonal_tavg
        processed['fluxBn_zonal_tavg'] = fluxBn_zonal_tavg
        ...
        fluxBn_nz_tavg = np.mean(fluxBn_nz[tavg_start_idx:tavg_end_idx, :], axis = 0)
        processed['keBn_nz_tavg'] = keBn_nz_tavg
        processed['enBn_nz_tavg'] = enBn_nz_tavg
        processed['fluxBn_nz_tavg'] = fluxBn_nz_tavg
