...
nu = 2e-4 #1e-3 #2e-4 #1e-4 #2e-4 #8e-5 #5e-5
gamma = 1920 #675 #85 #0 #1920 #240 #30
...
# to combine
processed_other = np.load('processed_zonal_profiles_' + output_suffix + '.npy', allow_pickle = True)[()]
subtasks_out_other = processed_other['subtasks']
labels_out_other = processed_other['labels']
...
for w in range(nwritesf):
    ...
    for taskout in tasks_out:
        ...
        for st in subtasks_out_other[taskout]:
            processed[idx + w][taskout]['data_' + st] = processed_other[idx + w][taskout]['data_' + st]
...
ymin_u_other = processed_other['ymin_u']
ymax_u_other = processed_other['ymax_u']
ymin_vort_other = processed_other['ymin_vort']
ymax_vort_other = processed_other['ymax_vort']

for tt in tasks_out:
    for st in subtasks_out_other[tt]:
        subtasks_out[tt].append(st)
    for ll in labels_out_other[tt]:
        labels_out[tt].append(ll)

ymin_u = np.min([ymin_u, ymin_u_other])
ymax_u = np.max([ymax_u, ymax_u_other])
ymin_vort = np.min([ymin_vort, ymin_vort_other])
ymax_vorT = np.max([ymax_vort, ymax_vort_other])
