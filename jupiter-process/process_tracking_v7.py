...
Options:
    ...
    --rho_half_width=<float>  half-width of CPC-centred profile window [default: 0.15]
...
rho_half_width = float(args['--rho_half_width'])
...
# ── profile grids ──────────────────────────────────────────────────────────────
# Full signed grid: rho in [-rho_half_width, rho_half_width], 2*n_r_profile - 1 points
# so that rho=0 is exactly represented and the spacing matches the folded grid.
# Folded grid: |rho| in [0, rho_half_width], n_r_profile points.
rho_profile_pts      = np.linspace(-rho_half_width, rho_half_width, 2*n_r_profile - 1)
rho_abs_profile_pts  = np.linspace(0, rho_half_width, n_r_profile)

if extract_vel:
    # full signed profiles
    uphi_CPC_sum  = np.zeros(2*n_r_profile - 1)
    ur_CPC_sum    = np.zeros(2*n_r_profile - 1)
    # folded (|rho|) profiles
    uphi_CPC_fold_sum = np.zeros(n_r_profile)
    ur_CPC_fold_sum   = np.zeros(n_r_profile)
    n_vel_profile     = 0
    n_vel_skipped     = 0
...
if subtract_rw:
    ...
    if extract_vel:
        uphi_CPC_sub_sum      = np.zeros(2*n_r_profile - 1)
        ur_CPC_sub_sum        = np.zeros(2*n_r_profile - 1)
        uphi_CPC_sub_fold_sum = np.zeros(n_r_profile)
        ur_CPC_sub_fold_sum   = np.zeros(n_r_profile)
        n_vel_sub             = 0
...
for i, w in enumerate(ws):
    ...
    if extract_vel and not glitch:
        ...
        # boundary check: skip if CPC centre is too close to pole or outer boundary
        r_bound = r_cutoff if use_cutoff else 1.0
        #if r_loc < rho_half_width or r_loc > r_bound - rho_half_width:
        #if r_loc < (r_cutoff/2 - rho_half_width) or r_loc > (r_bound - rho_half_width):
        
        if r_loc < 0.05:
            n_vel_skipped += 1
            print(f"  frame {i}: skipping profile (r_loc={r_loc:.4f})")# within "
                  #f"{rho_half_width:.3f} of boundary)")
            continue

        # CPC-frame velocity transformation
        ...
        # extract slice at phi closest to phi_CPC
        phi_slice_idx = np.argmin(np.abs(phi_1d - lon_loc))
        uphi_slice    = uphi_CPC_field[phi_slice_idx, :]   # function of r_1d
        ur_slice      = ur_CPC_field[phi_slice_idx, :]

        # rho = r - r_loc  (positive = away from pole)
        rho_1d = r_1d - r_loc

        # ── full signed profile: interpolate onto rho_profile_pts ─────────────
        uphi_CPC_sum += np.interp(rho_profile_pts, rho_1d, uphi_slice)
        ur_CPC_sum   += np.interp(rho_profile_pts, rho_1d, ur_slice)

        # ── folded profile: average outward (rho > 0) and inward (rho < 0) sides
        # u_phi is antisymmetric in rho: negate inward side before averaging
        # u_r   is symmetric   in rho: average directly
        # outward side: rho in [0, rho_half_width]
        uphi_out = np.interp(rho_abs_profile_pts,  rho_1d, uphi_slice)
        ur_out   = np.interp(rho_abs_profile_pts,  rho_1d, ur_slice)
        # inward side: rho in [-rho_half_width, 0]; reverse so argument is ascending
        uphi_in  = np.interp(rho_abs_profile_pts, -rho_1d[::-1], uphi_slice[::-1])
        ur_in    = np.interp(rho_abs_profile_pts, -rho_1d[::-1], ur_slice[::-1])
        uphi_CPC_fold_sum += 0.5 * (uphi_out - uphi_in)   # antisymmetric fold
        ur_CPC_fold_sum   += 0.5 * (ur_out   + ur_in)     # symmetric fold
        ...
        if subtract_rw:
            ...
            phi_shifted   = phi_mesh - phi_wave_inst
            ...
            uphi_CPC_sub_field = ur_sub * np.sin(dphi_field) + uphi_sub * np.cos(dphi_field)
            ur_CPC_sub_field   = ur_sub * np.cos(dphi_field) - uphi_sub * np.sin(dphi_field)

            uphi_sub_slice = uphi_CPC_sub_field[phi_slice_idx, :]
            ur_sub_slice   = ur_CPC_sub_field[phi_slice_idx, :]

            # full signed
            uphi_CPC_sub_sum += np.interp(rho_profile_pts, rho_1d, uphi_sub_slice)
            ur_CPC_sub_sum   += np.interp(rho_profile_pts, rho_1d, ur_sub_slice)

            # folded
            uphi_sub_out = np.interp(rho_abs_profile_pts,  rho_1d, uphi_sub_slice)
            ur_sub_out   = np.interp(rho_abs_profile_pts,  rho_1d, ur_sub_slice)
            uphi_sub_in  = np.interp(rho_abs_profile_pts, -rho_1d[::-1], uphi_sub_slice[::-1])
            ur_sub_in    = np.interp(rho_abs_profile_pts, -rho_1d[::-1], ur_sub_slice[::-1])
            uphi_CPC_sub_fold_sum += 0.5 * (uphi_sub_out - uphi_sub_in)
            ur_CPC_sub_fold_sum   += 0.5 * (ur_sub_out   + ur_sub_in)
...
print(extract_vel, n_vel_profile)
if extract_vel and n_vel_profile > 0:
    # full signed profiles (rho in [-rho_half_width, rho_half_width])
    processed['uphi_CPC_mean']      = uphi_CPC_sum      / n_vel_profile
    processed['ur_CPC_mean']        = ur_CPC_sum        / n_vel_profile
    processed['rho_profile_pts']    = rho_profile_pts
    # folded profiles (|rho| in [0, rho_half_width])
    processed['uphi_CPC_fold_mean'] = uphi_CPC_fold_sum / n_vel_profile
    processed['ur_CPC_fold_mean']   = ur_CPC_fold_sum   / n_vel_profile
    processed['rho_abs_profile_pts'] = rho_abs_profile_pts
    # metadata
    processed['rho_half_width']     = rho_half_width
    processed['n_vel_profile']      = n_vel_profile
    processed['n_vel_skipped']      = n_vel_skipped
    print(f"Velocity profile averaged over {n_vel_profile} frames "
          f"({n_vel_skipped} skipped due to boundary proximity).")
    if subtract_rw and n_vel_sub > 0:
        ...
        processed['uphi_CPC_sub_fold_mean'] = uphi_CPC_sub_fold_sum / n_vel_sub
        processed['ur_CPC_sub_fold_mean']   = ur_CPC_sub_fold_sum   / n_vel_sub
