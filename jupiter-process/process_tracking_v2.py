...
Options:
    ...
    --max_jump_phi=<float>   max allowed jump in phi (rad) between frames before flagging as glitch [default: 2.0]
    --jump_vort_fac=<float>  if jump detected, only accept if new vorticity exceeds old by this factor [default: 5.0]

    --extract_vel=<bool>     extract CPC velocity profile at each write [default: True]
    --n_r_profile=<int>      number of radial points to sample for velocity profile [default: 256]
"""
...
print(args['--max_jump_phi'])
max_jump_phi  = float(args['--max_jump_phi'])
print(args['--jump_vort_fac'])
