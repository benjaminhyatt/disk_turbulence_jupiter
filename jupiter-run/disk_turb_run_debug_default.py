...
# Problem
problem = d3.IVP([p, u, tau_u, tau_p, wz, wz_lap, wz_tau, wz_adv, wz_for, wz_dam], namespace=locals())
...
analysis = solver.evaluator.add_file_handler('analysis_debug_default_aux', sim_dt = 0.05)
...
# main analysis tasks
analysis_new = solver.evaluator.add_file_handler('analysis_debug_default', sim_dt = 0.05)

# profile
vortm0 = d3.Average(vort, coords['phi'])
analysis_new.add_task(vortm0, name = 'w0')
drvortm0 = d3.Average(er@d3.grad(vort), coords['phi'])
analysis_new.add_task(drvortm0, name='drw0')
urm0 = d3.Average(ur, coords['phi'])
analysis_new.add_task(urm0, name='ur0')
analysis_new.add_task(urm0 * drvortm0, name='ur0_drw0')

# scalar 
analysis_new.add_task(d3.integ(vort), name = 'W')
analysis_new.add_task(drvortm0(r=1), name='drw0_1')
Fphim0 = d3.Average(ephi@F, coords['phi'])
analysis_new.add_task(-Fphim0(r=1), name='F_1')

# coeff space
analysis_new.add_task(tau_u, layout='c', name='tau_u')
analysis_new.add_task(u*vort, layout='c', name='u_wz_c') 
analysis_new.add_task(F, layout='c', name='F_c') # is this working as desired?
analysis_new.add_task(d3.lap(u), layout='c', name='lap_u_c')
analysis_new.add_task(u, layout='c', name='u_c') # for new lap attempt
# snapshots
analysis_new.add_task(vort, layout='g', name='vorticity') # check for stability
