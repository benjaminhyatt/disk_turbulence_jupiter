...
if not implicit:
    ...
    if tau_mod:
        if restart_eps_0:
            problem.add_equation("dt(u) - nu*lap(u) + grad(p) + G*d3.skew(u) + lift(tau_u) + sig*lift_2(tau_u) = - u@grad(u) - alpha*u")
        else:
            problem.add_equation("dt(u) - nu*lap(u) + grad(p) + G*d3.skew(u) + lift(tau_u) + sig*lift_2(tau_u) = - u@grad(u) + amp*F - alpha*u")
    else:
        if restart_eps_0:
            problem.add_equation("dt(u) - nu*lap(u) + grad(p) + G*d3.skew(u) + lift(tau_u) = - u@grad(u) - alpha*u")
        else:
            problem.add_equation("dt(u) - nu*lap(u) + grad(p) + G*d3.skew(u) + lift(tau_u) = - u@grad(u) + amp*F - alpha*u")
...
analysis = solver.evaluator.add_file_handler('analysis_' + output_suffix, sim_dt = 1e-5, mode=file_handler_mode)
...
CFL = d3.CFL(solver, initial_dt=tstep, cadence=1, safety=safety, threshold=0.05, max_dt=max_dt_choice)
    ...
    while solver.proceed:
        ...
        if (solver.iteration-1) % 10 == 0:
            ...
            if max_u > 1e4 or np.isnan(max_u):
                ...
                break
        if solver.sim_time >= t_switch and switch:
