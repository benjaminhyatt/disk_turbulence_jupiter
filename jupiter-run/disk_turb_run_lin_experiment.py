...
#processed_profiles = np.load('../jupiter-process/4_24_26/processed_profiles_nu_2em04_gam_9d5ep02_kf_2d0ep01_Nphi_512_Nr_256_eps_1d0ep00_alpha_1d0em02_ring_0_restart_evolved_0_tau_mod_1_seed_31415926_safety_1d0em01_timestepper_SBDF2_bc_sf.npy', allow_pickle=True)[()]
processed_profiles = np.load('../jupiter-process/processed_profiles_filtered_nom1_nu_2em04_gam_9d5ep02_kf_2d0ep01_Nphi_512_Nr_256_eps_1d0ep00_alpha_1d0em02_ring_0_restart_evolved_0_tau_mod_1_seed_31415926_safety_1d0em01_timestepper_SBDF2_bc_sf.npy', allow_pickle=True)[()]
#u0['g'][0] = np.copy(processed_profiles['um0_tavg'])
#for ii in range(u0['g'].shape[1]):
#    u0['g'][0][ii, :] = processed_profiles['um0_tavg'] 
u0_global_grid_data = np.zeros((2, int(dealias*Nphi), int(dealias*Nr)))
for ii in range(int(dealias*Nphi)):
    u0_global_grid_data[0, ii, :] = processed_profiles['um0_tavg']

u0.load_from_global_grid_data(u0_global_grid_data)
u0.low_pass_filter(scales = 0.2)
...
#u0full = dist.Field(bases=disk)
#u0full.change_scales(dealias) 
#u0full['g'] += np.copy(u0['g'] + 10 * u_psi(psi)['g'])
...
problem.add_equation("dt(u) - nu*lap(u) + grad(p) + lift(tau_u) = -u0@grad(u) - u@grad(u0) - alpha*u - G*d3.skew(u)")
#problem.add_equation("dt(u) - nu*lap(u) + grad(p) + lift(tau_u) = -u0@grad(u) - u@grad(u0) - alpha*u - G*d3.skew(u) -u@grad(u)")
...
# Analysis
analysis = solver.evaluator.add_file_handler('analysis_lin_u0ivp', sim_dt = 0.005, mode='overwrite')
...
#analysis.add_task(u - u0, layout='g', name='u')

vm1 = dist.Field(bases=disk)
def vortm1_func():
    vort_ivp = vort.evaluate()
    vort_ivp.change_scales(dealias)
    vm1.change_scales(dealias)
    vm1['c'][4:6, :] = np.copy(vort_ivp['c'][4:6, :])
    return vm1['g']
    
vortm1 = d3.GeneralFunction(dist, vort.domain, vort.tensorsig, dtype, 'g', vortm1_func)
vortm1_f = dist.Field(bases=disk, name = 'vortm1_f')
analysis.add_task(vortm1, name='vortm1_f')
